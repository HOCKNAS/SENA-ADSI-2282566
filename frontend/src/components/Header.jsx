		...
		<nav className='flex justify-between w-full px-5 navbar menu'>
			...
			<div>
				<ul className='inline-flex w-full py-6 my-3 space-x-4'>
					...
					<li><button onClick={() => cerrarSesion()} className='flex w-full p-4 px-2 my-3 text-white bg-blue-900 rounded-md hover:bg-blue-600 justify-items-stretch fas fa-sign-out-alt '>  Cerrar Sesión  </button></li>
				</ul>
			</div >
		</nav >
	);
...
const Ruta = ({ icono, ruta, nombre, usuario }) => {
	...
	return (
		<Link to={ruta}>
			<button
				className={`flex w-full p-4 my-2 bg-${isActive ? 'blue' : 'gray'}-900 hover:bg-blue-600 justify-items-stretch text-white rounded-md border-red-900 solid`}>
